...
async def main():    
    ...
    for id in chat_ids:
        ...
        try:
            ...
            all_coins_message_count_in_6_hours.append(np.nan)
            continue
        messages = await client.get_messages(channel_entity, limit= 60,offset_date = datetime.now(),wait_time = 1) # Message limit is 60 here
        
        # Iterate through each message to get messages' time
        for msg in messages:
        	#print(msg)
